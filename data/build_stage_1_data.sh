...
python subsample_data.py --path  $DATAFOLDER/'data.jsonl' --author_splits_path $DATAFOLDER/'processed_data_stage1/author_splits.json'  --per_author 10

# 3) Split the data into smaller files
python split_files.py --dir $DATAFOLDER/'processed_data_stage1' --max_per_file 50000
...
CUDA_VISIBLE_DEVICES=0 python generate_paraphrases.py --in_dir $DATAFOLDER/'processed_data_stage1/split_files' --out_dir $DATAFOLDER/'processed_data_stage1/paraphrased_files' --temp 1.5 --top_p 0.8 --idx 0 --num_workers $WORKERS --batch_size 32 --max_input_length 60 --max_output_length 60 &
# CUDA_VISIBLE_DEVICES=1 python generate_paraphrases.py --in_dir $DATAFOLDER/'processed_data_stage1/split_files' --out_dir $DATAFOLDER/'processed_data_stage1/paraphrased_files' --temp 1.5 --top_p 0.8 --idx 1 --num_workers $WORKERS --batch_size 32 --max_input_length 60 --max_output_length 60 &

wait # Wait for all workers to finish

# 5) Convert data to the dataset format
python convert_to_dataset_format.py --in_dir  $DATAFOLDER/'processed_data_stage1/paraphrased_files/topp0.8_tmp1.5' --out_dir $DATAFOLDER/'processed_data_stage1/dataset_format' --name authorship_dataset

# 6) Preprocess the data to add Wegmann embeddings
python preprocess_add_source_text_embeddings.py --dataset_path $DATAFOLDER/'processed_data_stage1/dataset_format/authorship_data --style_model_name "AnnaWegmann/Style-Embedding"
